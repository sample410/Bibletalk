...
export default class FocusAreaFooter {
	...
	constructor( footer ) {
		...
		this.$menu = this.$footer.find( '.ps-focus__menu' );
		this.$menuInner = this.$menu.find( '.ps-focus__menu-inner' );
		this.$menuItems = this.$menu.find( '.ps-focus__menu-item' );
		this.$menuMore = this.$menuItems.filter( '.ps-js-focus-link-more' );
		...
		$( window ).on( `resize.${ EVT_SUFFIX }`, _.debounce( () => this.rearrange(), 1000 ) );
	}
	...
	centerActiveMenu() {
		...
		let $viewport = this.$menuInner,
			$active = this.$menuItems.filter( '.current' ),
	...
	rearrange() {
		...
		let maxWidth = this.$footer.width() - this.$interactions.width(),
			maxIterations = 20,
			iterations = 0,
			$last;
		...
		while ( ++iterations <= maxIterations && this.$menuInner.width() > maxWidth ) {
			...
			$last = this.$menuMore.prevAll( '.ps-focus__menu-item:visible' ).first();
			if ( $last.length ) {
				// Clone the last visible item.
				$last = $last.hide().clone();
				// Add cloned item to the dropdown.
